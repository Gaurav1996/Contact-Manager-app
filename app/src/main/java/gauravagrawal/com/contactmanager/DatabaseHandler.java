...
/**
 * Created by PC on 23-Jan-16.
 */
public class DatabaseHandler extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 1;
    private static final String DATABASE_NAME = "contactManager", TABLE_CONTACTS = "contacts", KEY_ID = "id", KEY_NAME = "name", KEY_PHONE = "phone",
            KEY_EMAIL = "email", KEY_ADDRESS = "address", KEY_IMAGEURI = "imageuri";
    ...
    public void createContact(Contact contact){//Creating the contact
    ...
    public Contact getContact(int id){//Reading the Contact
    ...
    public void deleteContact(Contact contact){//Deleting the Contact
        SQLiteDatabase db=getWritableDatabase();
        db.delete(TABLE_CONTACTS,KEY_ID+"=?",new String[]{String.valueOf(contact.getId())});
        db.close();
    }
    public int getContactsCount(){//Number of Contacts
        SQLiteDatabase db=getReadableDatabase();
    ...
    public int updateContact(Contact contact){//Updating contacts
        SQLiteDatabase db=getWritableDatabase();
        ContentValues values=new ContentValues();
        values.put(KEY_NAME,contact.getname());
        values.put(KEY_PHONE,contact.getphone());
